...
public class TapLabel: UILabel, NSLayoutManagerDelegate {
    ...
    public override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?)
    {
        ...
        let previousLocation = (touches.first! as UITouch).locationInView(self)
        if let (link, range) = linkAtPoint(previousLocation) {
            selected = (link, range)
        } else {
    ...
    public override func touchesMoved(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if (point?.x > previousPoint?.x || point?.x < previousPoint?.x || point?.y > previousPoint?.y || point?.y < previousPoint?.y) {
            isTouchMoved = true
            selected = nil
